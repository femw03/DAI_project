...
UPPER_FOV_LIDAR = 15.0
LOWER_FOV_LIDAR = -40.0
CHANNELS_LIDAR = 64.0
RANGE_LIDAR = 100.00
POINTS_PER_SEC_LIDAR = 60000
FRAMERATE = 0.05
FRAME_PER_SEC = 20
...
trafficLight_bbox_record = False
count = 0
...
dir_trafficLightBox = 'TrafficLightBBox/'
...
# ==============================================================================
# -- TrafficLightBoundingBoxes ---------------------------------------------------
# ==============================================================================


class TrafficLightBoundingBoxes(object):
    """
    This class is responsible for creating 3D bounding boxes for traffic lights
    and drawing them client-side on the pygame surface.
    """

    @staticmethod
    def get_bounding_boxes(traffic_lights, camera):
        """
        Creates 3D bounding boxes based on carla traffic light list and camera.
        """

        bounding_boxes = [TrafficLightBoundingBoxes.get_bounding_box(traffic_light, camera) for traffic_light in traffic_lights]
        # filter objects behind camera
        bounding_boxes = [bb for bb in bounding_boxes if all(bb[:, 2] > 0)]
        return bounding_boxes
    ...
    def draw_bounding_boxes(display, bounding_boxes):
        ...
        Draws bounding boxes on pygame display.
        """
        global trafficLight_bbox_record
        global count

        bb_surface = pygame.Surface((VIEW_WIDTH, VIEW_HEIGHT))
        bb_surface.set_colorkey((0, 0, 0))

        if trafficLight_bbox_record == True:
            f = open("TrafficLightBBox/bbox"+str(count), 'w')
            print("TrafficLightBBox")
        for bbox in bounding_boxes:
            points = [(int(bbox[i, 0]), int(bbox[i, 1])) for i in range(8)]
    ...
    @staticmethod
    def get_bounding_box(traffic_light, camera):
        """
        Returns 3D bounding box for a traffic light based on camera view.
        """

        bb_cords = TrafficLightBoundingBoxes._create_bb_points(traffic_light)
        cords_x_y_z = TrafficLightBoundingBoxes._traffic_light_to_sensor(bb_cords, traffic_light, camera)[:3, :]
        cords_y_minus_z_x = np.concatenate([cords_x_y_z[1, :], -cords_x_y_z[2, :], cords_x_y_z[0, :]])
        bbox = np.transpose(np.dot(camera.calibration, cords_y_minus_z_x))
        camera_bbox = np.concatenate([bbox[:, 0] / bbox[:, 2], bbox[:, 1] / bbox[:, 2], bbox[:, 2]], axis=1)
        return camera_bbox
    ...
    @staticmethod
    def _create_bb_points(traffic_light):
        """
        Returns 3D bounding box for a traffic light.
        """

        cords = np.zeros((8, 4))
        extent = traffic_light.bounding_box.extent
        cords[0, :] = np.array([extent.x, extent.y, -extent.z, 1])
        cords[1, :] = np.array([-extent.x, extent.y, -extent.z, 1])
        cords[2, :] = np.array([-extent.x, -extent.y, -extent.z, 1])
        cords[3, :] = np.array([extent.x, -extent.y, -extent.z, 1])
    ...
    @staticmethod
    def _traffic_light_to_sensor(cords, traffic_light, sensor):
        """
        Transforms coordinates of a traffic light bounding box to sensor.
        """
        world_cord = TrafficLightBoundingBoxes._traffic_light_to_world(cords, traffic_light)
        sensor_cord = TrafficLightBoundingBoxes._world_to_sensor(world_cord, sensor)
        return sensor_cord

    @staticmethod
    def _traffic_light_to_world(cords, traffic_light):
        """
        Transforms coordinates of a traffic light bounding box to world.
        """
        bb_transform = carla.Transform(traffic_light.bounding_box.location)
        bb_traffic_light_matrix = TrafficLightBoundingBoxes.get_matrix(bb_transform)
        traffic_light_world_matrix = TrafficLightBoundingBoxes.get_matrix(traffic_light.get_transform())
        bb_world_matrix = np.dot(traffic_light_world_matrix, bb_traffic_light_matrix)
        world_cords = np.dot(bb_world_matrix, np.transpose(cords))
        return world_cords
    ...
    def _world_to_sensor(cords, sensor):
        ...
        Transforms world coordinates to sensor.
        """
        sensor_world_matrix = TrafficLightBoundingBoxes.get_matrix(sensor.get_transform())
        world_sensor_matrix = np.linalg.inv(sensor_world_matrix)
        sensor_cords = np.dot(world_sensor_matrix, cords)
        return sensor_cords
    ...
    def game_loop(self, args):
        ...
        try:
            ...
            while True:
                ...
                trafficLights_bounding_boxes = TrafficLightBoundingBoxes.get_bounding_boxes(traffic_lights, self.camera)
                ...
                TrafficLightBoundingBoxes.draw_bounding_boxes(self.display, trafficLights_bounding_boxes)
