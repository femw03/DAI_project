...
class AdaptiveCruiseControlEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.distance_to_stop = np.random.uniform(
            30, 60
        )  # Random distance to stop line
    ...
    def step(self, action):
        ...
        if action[0] <= 0.5:
            ...
            self.agent_speed = max(
                0.0, self.agent_speed - brake_intensity * 10.0 * self.dt
            )
        else:
        ...
        if (
            self.time_step_counter % 600 == 0
        ):  # Every 600 steps, the car might disappear
        ...
        self.relative_distance += (
            self.current_target_speed - self.agent_speed
        ) * self.dt

        # Introduce random stopping line at least 30m ahead
        if self.time_step_counter % 300 == 0 and np.random.rand() < 0.3:
            self.stop = 1
            self.distance_to_stop = 30 + np.random.uniform(
                0, 30
            )  # Ensure stop line is at least 30m ahead
        ...
        terminated = (
            self.No_stop or self.relative_distance < 0.5
        )  # Terminate if stopped at the line
    ...
    def render(self):
        ...
        if self.current_target_speed > 0:
            pygame.draw.rect(
                self.screen,
                (0, 0, 255),
                (target_x, self.screen_height // 2, self.car_width, self.car_height),
            )
        ...
        pygame.draw.rect(
            self.screen,
            (255, 0, 0),
            (agent_x, self.screen_height // 2, self.car_width, self.car_height),
        )
        ...
        if self.stop == 1:
            ...
            pygame.draw.line(
                self.screen,
                (0, 255, 0),
                (stop_line_x, self.screen_height // 2 - 10),
                (stop_line_x, self.screen_height // 2 + 30),
                5,
            )
        ...
        distance_text = font.render(
            f"Distance: {self.relative_distance:.1f} m", True, (0, 0, 0)
        )
        agent_speed_text = font.render(
            f"Agent Speed: {self.agent_speed:.1f} m/s", True, (0, 0, 0)
        )
        action_text = font.render(
            f"Action: {action_type} ({self.last_action:.2f})", True, (0, 0, 0)
        )  # Display actual action value
        target_speed_text = font.render(
            f"Lead vehicle speed: {self.current_target_speed}", True, (0, 0, 0)
        )
    ...
    def _get_reward(self) -> float:
        ...
        if self.stop == 1:  # If stop flag is set
            ...
            ):  # Factor in the current speed to promote smooth stopping
                speed_factor = max(0, 1 - self.agent_speed / self.max_speed)
                distance_factor = 1 - min(
                    1, self.distance_to_stop / 30
                )  # Scale down as the distance to stop decreases
                stop_reward = (
                    0.5 * distance_factor + 0.5 * speed_factor
