...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        total_obs_dim = (
            self.static_dim + self.task_dim + self.max_objects * self.object_feature_dim
        )
        ...
        )  # low = full brake          high = full throttle
    ...
    def reset(self, seed=None, **kwargs):
        ...
        self.task_features = np.zeros(
            self.task_dim, dtype=np.float32
        )  # Flatten task features

        # Generate random number of objects (variable length)
        num_objects = self.max_objects
        self.object_features = np.zeros(
            (num_objects, self.object_feature_dim), dtype=np.float32
        )
        ...
        self.current_observation = np.concatenate(
            [
                ...
                self.task_features.flatten(),
                self.object_features.flatten(),
            ]
        )
    ...
    def step(self, action):
        ...
        if self.perfect:
            self.current_observation = (
                self._get_perfect_obs()
            )  # use observation from Carla
        else:
            self.current_observation = (
                self._get_obs()
            )  # use observation from computer vision
    ...
    def _get_perfect_obs(self):
        ...
        steering_angle = get_steering_angle(self.world)
        ...
        if len(filtered_objects) > self.max_objects:
            ...
            padding = [
                Object(
                    type=random.randint(0, 1), confidence=0.0, distance=0.0, angle=0.0
                )
    ...
    def _get_obs(self):
        ...
        object_features = np.random.uniform(
            -1, 1, size=(num_objects, self.object_feature_dim)
        )

        # Padding to ensure we have max_objects number of objects
        padded_objects = np.zeros(
            (self.max_objects, self.object_feature_dim), dtype=np.float32
        )
        padded_objects[:num_objects] = object_features

        # Flatten the entire observation into a single vector
        observation = np.concatenate(
            [static_features, task_features, padded_objects.flatten()]
        )
    ...
    def _get_reward(self, action):
        ...
        for object in filtered_objects:
            ...
            if static_features[1] < 0.1:      # Car is considered stopped: remove stop flag
              ...
              pedestrian_reward = -0.1*static_features[1]       # Penalty for moving when a stop is required"""
                ...
                if (
                    abs(object.angle) <= np.radians(2) and object.distance != 0
                ):  # angle of attack = +-2°
                    ...
                    if (current_speed / 3.6) > 1:
                        safe_distance = (
                            2 * (current_speed / 3.6)
                        )  # Approximation of 2 seconds * current_speed note, this rule is in m/s not km/h!
                    else:
                    ...
                    if (
                        current_speed > 2
                    ):  # we don't want him to decide to crash if he is standing stationary a little to close! because otherwise he will collect -5 for each timestep!
                        if (
                            object.distance <= safe_distance - distance_margin
                            and object.distance >= 0.001
                        ):
                            safe_distance_reward = max(
                                -5,
                                -0.5
                                * ((safe_distance - distance_margin) / object.distance),
                            )  # Large penalty for being too close (unsafe)

                        elif (
                            object.distance < safe_distance + distance_margin
                            and object.distance > safe_distance - distance_margin
                        ):
                            safe_distance_reward = (
                                1  # Bonus for staying within 15% of safe distance
                            )

                        elif current_speed < speed_limit - speed_margin:
                            safe_distance_reward = max(
                                -1, -0.1 * (object.distance / safe_distance)
                            )  # Smaller penalty for trailing too far behind necessary since we allready punish going to slow?
                        # print("safe_distance_reward: ", safe_distance_reward)

                else:  # No vehicle in angle-of-attack
                    """Driving too slow"""
                    if (
                        speed_limit - speed_margin > current_speed
                        and current_speed >= 0.1
                    ):
      ...
      stop_reward = 0"""
        ...
        wandb.log(
            {
                "slow_speed_reward": slow_speed_reward,
