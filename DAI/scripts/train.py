...
def main():
    ...
    model = SAC.load("/mnt/storage/resultsRL/LeadingCar_cv_perfect_smooth2_20000.zip", env=env, verbose=1)
    print("loaded: ", model)
    ...
    for step in range(0, total_timesteps, n_steps):
        ...
        model.save(f"/mnt/storage/resultsRL/LeadingCar_cv_segmentaion_smooth2_{step + n_steps}")
        wandb.save(
            f"/mnt/storage/resultsRL/LeadingCar_cv_segmentation_smooth2_{step + n_steps}.zip"
        )
        print(f"Model saved at step: {step + n_steps}")

    # Save the final model
    model.save("LeadingCar_cv_segmentation_smooth2_final")
    wandb.save("LeadingCar_cv_segmentation_smooth2_final.zip")
